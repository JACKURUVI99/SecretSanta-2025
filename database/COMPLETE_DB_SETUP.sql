...
CREATE POLICY "Admins manage pairings" ON public.pairings FOR ALL USING (
    EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_admin = TRUE)
);
DROP POLICY IF EXISTS "Users view their target" ON public.pairings;
CREATE POLICY "Users view their target" ON public.pairings FOR SELECT USING (
    auth.uid() = user_id
);
...
END;
$$;
DROP FUNCTION IF EXISTS public.submit_bonus_task(UUID, UUID, JSONB);
