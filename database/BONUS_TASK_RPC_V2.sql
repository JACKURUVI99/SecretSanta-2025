BEGIN;
CREATE OR REPLACE FUNCTION public.get_bonus_task_v2(p_user_id UUID)
RETURNS TABLE (
    task_id UUID,
    task_title TEXT,
...
BEGIN
    ...
    FROM bonus_tasks bt
    LEFT JOIN (
        SELECT uts.task_id, COUNT(*) as attempts_count, MAX(uts.score) as max_score_achieved
        FROM user_task_submissions uts
        WHERE uts.user_id = p_user_id
        GROUP BY uts.task_id
