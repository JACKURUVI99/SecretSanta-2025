...
export default function ViewProfileModal({ userId, onClose }: ViewProfileModalProps) {
    ...
    const toggleLike = async () => {
        ...
        if (hasLiked) {
            // Unlike
            await supabase.from('profile_likes').delete().eq('user_id', user.id).eq('target_id', userId);
            setLikes(l => l - 1);
            setHasLiked(false);
        } else {
            // Like
            await supabase.from('profile_likes').insert({ user_id: user.id, target_id: userId });
            setLikes(l => l + 1);
            setHasLiked(true);
        }
    };
